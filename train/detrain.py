...
import preprocessing.prepros as prepros  # prepros가 preprocessing 폴더에 있음

# import VGGPerceptual (필요 시 주석 해제 및 경로 수정)
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(save_dir, "dedrop_derain_dehaze.pt")
    ...
    subset_size = int(dataset_size * 0.1) # 10% 사용 (정수형 변환 명시)
    if subset_size == 0 and dataset_size > 0: subset_size = 1 # 데이터가 너무 적을 경우 최소 1개 보장
    ...
    loader = DataLoader(
        ...
        shuffle=True,
        num_workers=0,    # Windows 호환성을 위해 0 권장 (필요 시 1로 변경)
        pin_memory=True
    )
    ...
    model.eval()
    example = torch.randn(1, 3, img_h, img_w).to(device) # device에 맞게 입력 생성
